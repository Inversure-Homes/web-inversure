...
def _assert_project_metrics(
    ...
    _assert_money(
        service_project["beneficio_inversure"], expected_study["comision_inversure_eur"], label="dashboard.comision"
    )

    _assert_money(
        detail_context["captacion"]["capital_objetivo"], expected_detail["capital_objetivo"], label="capital objetivo"
    )
    _assert_money(
        detail_context["captacion"]["capital_captado"], expected_detail["capital_captado"], label="capital captado"
    )
    _assert_money(
        detail_context["captacion"]["restante"], expected_detail["capital_pendiente"], label="capital pendiente"
    )
    _assert_pct(
    ...
    _assert_money(
        actual_study["comision_inversure_eur"], expected_study["comision_inversure_eur"], label="estudio.comision"
    )
    _assert_money(
        actual_study["beneficio_neto_inversor"], expected_study["beneficio_neto_inversor"], label="estudio.beneficio"
    )
    ...
    _assert_money(
        actual_pdf["beneficio_neto_real"], expected_pdf["beneficio_neto_real"], label="pdf.beneficio_neto_real"
    )
    _assert_money(actual_pdf["comision_inversure_real"], expected_pdf["comision_inversure_real"], label="pdf.comision")
    _assert_money(
        actual_pdf["impuesto_sociedades_real"], expected_pdf["impuesto_sociedades_real"], label="pdf.impuesto"
    )
    _assert_money(
    ...
    _assert_pct(
        actual_pdf["roi_real_tras_impuestos"],
        expected_pdf["roi_real_tras_impuestos"],
        label="pdf.roi_real_tras_impuestos",
    )
    ...
    _assert_money(
        actual_settlement["beneficio_neto"], expected_settlement["neto_cobrar"], label="dashboard.neto_cobrar"
    )
    _assert_money(
        actual_settlement["retorno_total"], expected_settlement["total_a_percibir"], label="dashboard.total_a_percibir"
    )
...
def test_financing_pct_is_metadata_only(verified_client, direccion_user):
    ...
    with _freeze_financial_now():
        rentable_service = _service_payload(
            direccion_user, filters=FinancialDashboardFilters.from_mapping(rentable_query)
        )
        financed_service = _service_payload(
            direccion_user, filters=FinancialDashboardFilters.from_mapping(financed_query)
        )
    ...
    for key in (
        ...
        _assert_money(
            rentable_detail["resultado"][key], financed_detail["resultado"][key], label=f"detalle.financing.{key}"
        )
    for key in ("roi", "margen_neto"):
        _assert_pct(
            rentable_detail["resultado"][key], financed_detail["resultado"][key], label=f"detalle.financing.{key}"
    ...
    for key in (
        ...
        _assert_money(
            rentable_pdf["resumen"][pdf_key_map[key]],
            financed_pdf["resumen"][pdf_key_map[key]],
            label=f"pdf.financing.{key}",
        )
...
def test_partial_contributions_keep_same_client_movements_and_date_filtered_series(verified_client, direccion_user):
    ...
    assert [row["beneficio"] for row in service_payload["series"]["monthly"]["performance"]] == [
        Decimal("0"),
        Decimal("0"),
    ]
...
def test_closed_project_distinguishes_project_pdf_and_liquidation_tax_bases(verified_client, direccion_user):
    ...
    _assert_pct(
        detail_context["inv"]["roi_neto_tras_impuestos"], Decimal("33.75"), label="estudio.roi_neto_tras_impuestos"
    )
    _assert_pct(
        pdf_context["resumen"]["roi_real_tras_impuestos"],
        Decimal("30.33707865168539"),
        label="pdf.roi_real_tras_impuestos",
    )
    _assert_money(
        project_metrics["investment_return"]["retorno_total"], Decimal("136450.00"), label="dashboard.total_a_percibir"
    )
    _assert_money(
        liquidation_response.json()["resumen"]["total_a_percibir"],
        Decimal("127337.50"),
