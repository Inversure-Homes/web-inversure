...
def _build_comunicacion_context(
    ...
    # Fuente de verdad: `_resultado_desde_memoria` (es lo que usa la vista de proyecto/dashboard backend).
    # Fallbacks: valores del snapshot/económico si no hay movimientos.
    ...
    rent_est = (
        resultado_mem.get("roi")
        or econ.get("roi_estimado")
        or econ.get("roi")
        or snap_met.get("roi_estimado")
        or snap_met.get("roi")
...
def _roi_memoria_proyecto(proyecto: Proyecto):
    gastos = list(GastoProyecto.objects.filter(proyecto=proyecto))
    ingresos = list(IngresoProyecto.objects.filter(proyecto=proyecto))
...
def guardar_proyecto(request, proyecto_id: int):
    ...
    if mostrar_landing_raw is not None and (_user_can_edit_project(request.user, proyecto_obj) or _user_can_manage_publicacion(request.user)):
...
def contrato_prestamo(request, proyecto_id: int, participacion_id: int):
    ...
    original traía el calendario de liquidaciones tecleado, y una fecha mal
    escrita en un documento firmado es un problema, no una errata.

    Con `?html=1` se ve el HTML sin pasar por WeasyPrint, para revisar el
    maquetado.
    """
    participacion = get_object_or_404(
        Participacion.objects.select_related("cliente", "proyecto"),
    ...
    if not _user_can_view_project(request.user, participacion.proyecto):
        messages.error(request, "No tienes acceso a este proyecto.")
        return redirect("core:lista_proyectos")
